...
const
  { Windows 2000/XP multimedia keys (adapted from winuser.h and renamed to
    avoid potential conflicts
    See also: http://msdn.microsoft.com/library/default.asp?url=/library/en-us/
    winui/winui/WindowsUserInterface/UserInput/VirtualKeyCodes.asp }
...
procedure Register;
begin
  RegisterComponents('RunSoft', [THotKeyManager]);
end;

//Desc: 初始化键盘布局
procedure InitKeyboardLayout;
var nIdx: integer;
    nLayNum: Cardinal;
    nLays: array[0..100] of HKL;
begin
  ...
  nLayNum := GetKeyboardLayoutList(100, nLays);
  ...
  for nIdx:=0 to nLayNum do
  if nLays[nIdx] = EnglishKeyboardLayout then
  begin
    ShouldUnloadEnglishKeyboardLayout := False; Break;
  end;  
end;
